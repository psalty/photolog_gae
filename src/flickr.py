...
def geo_getLocation(photo_id=''):
    """Returns a list of tags for the given user (in string format)"""
    method = 'flickr.photos.geo.getLocation'

    data = _doget(method, auth=False, photo_id=photo_id)
    loc = data.rsp.photo.location
    return [loc.latitude, loc.longitude]
...
def tags_getListUser(user_id=''):
    ...
    if isinstance(data.rsp.who.tags.tag, list):
        return [tag.text for tag in data.rsp.who.tags.tag]
    else:
        return [data.rsp.who.tags.tag.text]

def tags_getListPhoto(photo_id=''):
    """Returns a list of tags for the given user (in string format)"""
    ...
    if isinstance(data.rsp.photo.tags.tag, list):
        ...
        return [data.rsp.photo.tags.tag.text]
...
def _doget(method, auth=False, **params):
    ...
    url = '%s%s/?api_key=%s&method=%s&%s%s'% \
          (HOST, API, config.API_KEY, method, urlencode(params),
                  _get_auth_url_suffix(method, auth, params))

    #another useful debug print statement
    #print url
    # this is a change for google app engine
    result = urlfetch.fetch(url)
    return _get_data(minidom.parseString(result.content))
...
def _dopost(method, auth=False, **params):
    ...
    url = '%s%s/?api_key=%s&method=%s&%s'% \
          (HOST, API, config.API_KEY, method, _get_auth_url_suffix(method, auth, params))
    ...
    #print payload
    # this is a change for google app engine
    result = urlfetch.fetch(url,result = urlfetch.fetch(url,payload))
    return _get_data(minidom.parseString(result.content))
...
def _get_auth_url_suffix(method, auth, params):
    ...
    for item in paramaters:
        ...
        if item == 'API_KEY':
            ...
            api_string.append(config.API_KEY)
        if item == 'auth_token':
...
class Auth():
    def getFrob(self):
        ...
        sig_str = config.API_SECRET + 'api_key' + config.API_KEY + 'method' + method
    ...
    def loginLink(self, permission, frob):
        ...
        myAuth = Auth()
        sig_str = config.API_SECRET + 'api_key' + config.API_KEY + 'frob' + frob + 'perms' + permission
        signature_hash = hashlib.md5(sig_str).hexdigest()
        perms = permission
        link = "http://flickr.com/services/auth/?api_key=%s&perms=%s&frob=%s&api_sig=%s" % (config.API_KEY, perms, frob, signature_hash)
        return link

    def getToken(self, frob):
        """This token is what needs to be used in future API calls"""
        method = 'flickr.auth.getToken'
        sig_str = config.API_SECRET + 'api_key' + config.API_KEY + 'frob' + frob + 'method' + method
        signature_hash = hashlib.md5(sig_str).hexdigest()
        data = _doget(method, auth=False, api_sig=signature_hash, 
                      api_key=config.API_KEY, frob=frob)
